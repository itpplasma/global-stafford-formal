...
abbrev L : Type u := FractionRing (B (k := k) (n := n))

/-- The generic fibre `L ⊗[B] C`. -/
abbrev CL : Type u := L (k := k) (n := n) ⊗[B (k := k) (n := n)] C
...
theorem algebraMap_injective_of_etale :
    Function.Injective (algebraMap (B (k := k) (n := n)) C) := by
  intro a a' h
  by_contra hne
  ...
  have hzero : algebraMap (B (k := k) (n := n)) C (a - a') = 0 := by
...
theorem algebraMap_ne_zero_of_etale (b : B (k := k) (n := n)) (hb : b ≠ 0) :
    algebraMap (B (k := k) (n := n)) C b ≠ 0 := fun h0 =>
  hb (algebraMap_injective_of_etale (h0.trans (map_zero (algebraMap (B (k := k) (n := n)) C)).symm))
...
theorem finiteGenericFibre_of_etale :
    ∃ (m : ℕ) (c : Fin m → C), ∀ x : C, ∃ b : B (k := k) (n := n), b ≠ 0 ∧
      ∃ t : Fin m → B (k := k) (n := n),
        algebraMap (B (k := k) (n := n)) C b * x = ∑ i, c i * algebraMap (B (k := k) (n := n)) C (t i) := by
  classical
  -- `algebraMapSubmonoid C B⁰` consists of nonzerodivisors of `C`.
  have hsub : Algebra.algebraMapSubmonoid C (nonZeroDivisors (B (k := k) (n := n))) ≤
  ...
  haveI hloc : IsLocalization (Algebra.algebraMapSubmonoid C (nonZeroDivisors (B (k := k) (n := n))))
      (CL (k := k) (n := n) (C := C)) :=
    (Algebra.isLocalization_iff_isPushout (R := B (k := k) (n := n))
      (S := nonZeroDivisors (B (k := k) (n := n))) (A := L (k := k) (n := n))
  ...
  obtain ⟨m', g, hg⟩ := Module.Finite.exists_fin (R := L (k := k) (n := n))
    (M := CL (k := k) (n := n) (C := C))
  set S := Algebra.algebraMapSubmonoid C (nonZeroDivisors (B (k := k) (n := n))) with hS
  ...
  have hunit : ∀ j, IsUnit (algebraMap (B (k := k) (n := n)) (L (k := k) (n := n)) (denomB j)) :=
    fun j => IsLocalization.map_units (L (k := k) (n := n)) (⟨denomB j, hdenomB j⟩ :
      nonZeroDivisors (B (k := k) (n := n)))
  have hCLdenom : ∀ j, algebraMap C (CL (k := k) (n := n) (C := C)) (denom j : C) =
      algebraMap (L (k := k) (n := n)) (CL (k := k) (n := n) (C := C))
        (algebraMap (B (k := k) (n := n)) (L (k := k) (n := n)) (denomB j)) := by
    intro j
    rw [← hdenomBeq j, ← IsScalarTower.algebraMap_apply (B (k := k) (n := n)) C
        (CL (k := k) (n := n) (C := C)),
      IsScalarTower.algebraMap_apply (B (k := k) (n := n)) (L (k := k) (n := n))
        (CL (k := k) (n := n) (C := C))]
  ...
  have hgmem : ∀ j, g j ∈ Submodule.span (L (k := k) (n := n))
    ...
    intro j
    have hβ : (algebraMap (B (k := k) (n := n)) (L (k := k) (n := n)) (denomB j)) • g j =
        algebraMap C (CL (k := k) (n := n) (C := C)) (numer j) := by
      rw [Algebra.smul_def (algebraMap (B (k := k) (n := n)) (L (k := k) (n := n)) (denomB j))
        (g j), ← hCLdenom j]
      exact hspec j
  ...
  obtain ⟨bdiv, hbdiv⟩ := IsLocalization.exist_integer_multiples_of_finite
    (nonZeroDivisors (B (k := k) (n := n))) lam
  choose t ht using hbdiv
  refine ⟨(bdiv : B (k := k) (n := n)), nonZeroDivisors.ne_zero bdiv.2, t, ?_⟩
  apply hinjCtoCL
  have hlhs : algebraMap C (CL (k := k) (n := n) (C := C))
      (algebraMap (B (k := k) (n := n)) C (bdiv : B (k := k) (n := n)) * x) =
      (bdiv : B (k := k) (n := n)) • algebraMap C (CL (k := k) (n := n) (C := C)) x := by
    rw [Algebra.smul_def (bdiv : B (k := k) (n := n))
        (algebraMap C (CL (k := k) (n := n) (C := C)) x),
      map_mul,
      ← IsScalarTower.algebraMap_apply (B (k := k) (n := n)) C
        (CL (k := k) (n := n) (C := C)) (bdiv : B (k := k) (n := n))]
  rw [map_sum]
  have hrhs : ∀ j, algebraMap C (CL (k := k) (n := n) (C := C))
      (numer j * algebraMap (B (k := k) (n := n)) C (t j)) =
      ((bdiv : B (k := k) (n := n)) • lam j) • algebraMap C (CL (k := k) (n := n) (C := C))
        (numer j) := by
    intro j
    rw [mul_comm, map_mul,
      ← IsScalarTower.algebraMap_apply (B (k := k) (n := n)) C
      ...
      IsScalarTower.algebraMap_apply (B (k := k) (n := n)) (L (k := k) (n := n))
        (CL (k := k) (n := n) (C := C)) (t j),
      ht j,
      ← Algebra.smul_def ((bdiv : B (k := k) (n := n)) • lam j)
        (algebraMap C (CL (k := k) (n := n) (C := C)) (numer j))]
  simp only [hrhs]
  rw [hlhs, ← hlam, Finset.smul_sum]
  apply Finset.sum_congr rfl
  intro j _
  exact (smul_assoc (bdiv : B (k := k) (n := n)) (lam j)
